...
describe("API", () => {
  let token, registeredToken, registeredUser;
  let loginUser = { username: "glamgal", password: "glamgal123" };
  ...
  beforeAll(async () => {
    await rebuildDB();
    const { data } = await axios.post(`${API_URL}/api/users/login`, loginUser);
    token = data.token;
    loginUser = data.user;
  });
  afterAll(async () => {
  ...
  describe("Users", () => {
    ...
    describe("POST /users/login", () => {
      ...
      it("Returns a JSON Web Token. Stores the id and username in the token.", async () => {
        const parsedToken = jwt.verify(registeredToken, JWT_SECRET);
  ...
  describe("Activities", () => {
    ...
    describe("POST /activities (*)", () => {
      it("Creates a new activity", async () => {
        ...
        expect(respondedActivity.name).toEqual(activityToCreate.name);
        expect(respondedActivity.description).toEqual(
          activityToCreate.description
        );
        activityToCreate = respondedActivity;
      });
    });
  ...
  describe("Routines", () => {
    ...
    describe("POST /routines (*)", () => {
      it("Creates a new routine, with the creatorId matching the logged in user", async () => {
        ...
        expect(respondedRoutine.name).toEqual(routineToCreate.name);
        expect(respondedRoutine.goal).toEqual(routineToCreate.goal);
        expect(respondedRoutine.name).toEqual(routineToCreate.name);
        expect(respondedRoutine.creatorId).toEqual(2);
        routineToCreate = respondedRoutine;
      });
    ...
    describe("DELETE /routines/:routineId (**)", () => {
      it("Hard deletes a routine. Makes sure to delete all the routineActivities whose routine is the one being deleted.", async () => {
        const routineToDelete = await getRoutineById(4);
        console.log(routineToDelete);
        const { data: deletedRoutine } = await axios.delete(
          `${API_URL}/api/routines/${4}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        expect(deletedRoutine.id).toBe(routineToDelete.id);
        expect(deletedRoutine.name).toBe(routineToDelete.name);
        expect(deletedRoutine.goal).toBe(routineToDelete.goal);
        const shouldBeDeleted = await getRoutineById(routineToDelete.id);
  ...
  describe("routine_activities", () => {
    ...
    describe("PATCH /routine_activities/:routineActivityId (**)", () => {
      it("Updates the count or duration on the routine activity", async () => {
        //TODO : decouple from routineActivityToCreateAndUpdate
        const { data: respondedRoutineActivity } = await axios.patch(
          `${API_URL}/api/routine_activities/1`,
